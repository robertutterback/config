...
# #
# # DESCRIPTION: A user-friendly gdb configuration file, for x86/x86_64
# #and ARM platforms.
# #
...
# #
# # NOTES: 'help user' in gdb will list the commands/descriptions in
# #this file
# #        'context on' now enables auto-display of context screen
# #
# # MAC OS X NOTES: If you are using this on Mac OS X, you must either
# #attach gdb to a process
# #                 or launch gdb without any options and then load the
# #binary file you want to analyse with "exec-file" option
# #                 If you load the binary from the command line, like
# #$gdb binary-name, this will not work as it should
...
# #         and http://reverse.put.as/2009/08/26/gdb-patches/ (if you
# #want the fixed binary for i386)
# #
...
# #
# # iOS NOTES: iOS gdb from Cydia (and Apple's) suffer from the same OS
# #X bug.
# # If you are using this on Mac OS X or iOS, you must either attach gdb
# #to a process
# #            or launch gdb without any options and then load the
# #binary file you want to analyse with "exec-file" option
# #            If you load the binary from the command line, like $gdb
# #binary-name, this will not work as it should
# #            For more information, read it here
# #http://reverse.put.as/2008/11/28/apples-gdb-bug/
# #
...
# #     - Add commands header and loadcmds to dump Mach-O header
# #information
...
# #     - Add shortcuts to attach to VMware kernel debugging gdb stub
# #(kernel32 and kernel64)
# #
# #   Version 8.0.3 (21/03/2013)
# #  - Add option to colorize or not output (thanks to argp and skier
# #for the request and ideas!)
# #     - Convert the escape codes into functions so colors can be
# #easily customized
...
# #     - Merge pull request from mheistermann to support local
# #modifications in a .gdbinit.local file
# #     - Add a missing opcode to the stepo command
# #
# #   Version 8.0.1 (23/04/2012)
# #     - Small bug fix to the attsyntax and intelsyntax commands
# #(changing X86 flavor variable was missing)
# #
# #   Version 8.0 (13/04/2012)
# #     - Merged x86/x64 and ARM versions
# #     - Added commands intelsyntax and attsyntax to switch between x86
# #disassembly flavors
# #     - Added new configuration variables ARM, ARMOPCODES, and
# #X86FLAVOR
# #     - Code cleanups and fixes to the indentation
# #     - Bug fixes to some ARM related code
# #     - Added the dumpmacho command to memory dump the mach-o header
# #to a file
# #
# #   TODO:
# #
...
# # set to 1 to have ARM target debugging as default, use the "arm"
# # command to switch inside gdb
# set $ARM = 0
# # set to 0 if you have problems with the colorized prompt - reported
# # by Plouj with Ubuntu gdb 7.2
# set $COLOUREDPROMPT = 1
# # Colour the first line of the disassembly - default is green, if you
# # want to change it search for
...
# # set to 1 so skip command to execute the instruction at the new
# # location
# # by default it EIP/RIP will be modified and update the new context
# # but not execute the instruction
# set $SKIPEXECUTE = 0
# # if $SKIPEXECUTE is 1 configure the type of execution
# # 1 = use stepo (do not get into calls), 0 = use stepi (step into
# # calls)
# set $SKIPSTEP = 1
# # show the ARM opcodes - change to 0 if you don't want such thing (in
# # x/i command)
# set $ARMOPCODES = 1
...
# #          or just create a ~/.gdbinit.local and set these variables
# #          there
...
# # can also be used to redefine anything else in particular the colors
# # aka theming
...
#     # The A bit is set to 1 automatically. It is used to disable
# 		# imprecise data aborts. 
#     # It might not be writable in the Nonsecure state if the AW bit in
# 		# the SCR register is reset.
#     if ($cpsr->a & 1)
#         printf "A "
#         set $_a_flag = 1
#     else
...
#     # FIQ can be nonmaskable in the Nonsecure state if the FW bit in
# 		# SCR register is reset.
#     if ($cpsr->f & 1)
#         printf "F "
#         set $_f_flag = 1
#     else
...
#     # if 1 then the processor is executing in Thumb state or ThumbEE
# 		# state depending on the J bit
#     if ($cpsr->t & 1)
#         printf "T "
#         set $_t_flag = 1
#     else
...
#         # 4 bits are for condition codes (16 in total) (bits 31:28 in
# 			# ARM contain the condition or 1111 if instruction is
# 			# unconditional)
...
# 											dumpjumphelper
#         else
# 					# if bits 15-12 (opcode in Thumb instructions) are equal to
# 					# 1 1 0 1 (0xD) then we have a conditional branch
# 					# bits 11-8 for the conditional execution code (check ARMv7
# 					# manual A8.3)
...
# ##################### X86
#     else
#         ## grab the first two bytes from the instruction so we can
# 			## determine the jump instruction
#         set $_byte1 = *(unsigned char *)$pc
#         set $_byte2 = *(unsigned char *)($pc+1)
#         ## and now check what kind of jump we have (in case it's a
# 				## jump instruction)
#         ## I changed the flags routine to save the flag into a
# 				## variable, so we don't need to repeat the process :) (search
# 				## for "define flags")
...
# 											end 
#         end
#         ## opcode 0xE3: JCXZ, JECXZ, JRCXZ (jump if CX=0 or ECX=0 or
#         ## RCX=0)
#         if ($_byte1 == 0xE3)
# 					# cx=0 or ecx=0
# 					if (($ecx == 0) || ($cx == 0))
...
# #### I was thinking about how to do this and then it ocurred me that
# #it could be as simple as this ! :)
# define stepoframework
#     if $ARM == 1
#         # bl and bx opcodes
#         # bx Rn  => ARM bits 27-20: 0 0 0 1 0 0 1 0 , bits 7-4: 0 0 0
# 			# 1 ; Thumb bits: 15-7: 0 1 0 0 0 1 1 1 0
#         # blx Rn => ARM bits 27-20: 0 0 0 1 0 0 1 0 , bits 7-4: 0 0 1
# 			# 1 ; Thumb bits: 15-7: 0 1 0 0 0 1 1 1 1
#         # bl # => ARM bits 27-24: 1 0 1 1 ; Thumb bits: 15-11: 1 1 1 1
# 			# 0 
...
#         # Thumb Mode
#         else
#             # 32 bits instructions in Thumb are divided into two half
# 					# words
...
# 															end
#         end
#         # if we have found a call to bypass we set a temporary
# 				# breakpoint on next instruction and continue 
#         if ($_nextaddress != 0)
#             tbreak *$_nextaddress
#             continue
...
#         ## for the 0xFF opcodes, we can enumerate what is possible to
# 			## have
#         # first we grab the first 3 bytes from the current program
# 			# counter
#         set $_byte1 = *(unsigned char *)$pc
#         set $_byte2 = *(unsigned char *)($pc+1)
#         set $_byte3 = *(unsigned char *)($pc+2)
#         # and start the fun
#         # if it's a 0xE8 opcode, the total instruction size will be 5
# 				# bytes
#         # so we can simply calculate the next address and use a
# 				# temporary breakpoint ! Voila :)
#         set $_nextaddress = 0
#         # this one is the must useful for us !!!
#         if ($_byte1 == 0xE8)
#             set $_nextaddress = $pc + 0x5
#         else
#             # just other cases we might be interested in... maybe this
# 					# should be removed since the 0xE8 opcode is the one we will
# 					# use more
#             # this is a big fucking mess and can be improved for sure
# 					# :) I don't like the way it is ehehehe
#             if ($_byte1 == 0xFF)
#                 # call *%eax (0xFFD0) || call *%edx (0xFFD2) || call
# 							# *(%ecx) (0xFFD1) || call (%eax) (0xFF10) || call *%esi
# 							# (0xFFD6) || call *%ebx (0xFFD3) || call   DWORD PTR
# 							# [edx] (0xFF12)
...
#                 end
#                 # call *0x??(%ebp) (0xFF55??) || call *0x??(%esi)
# 								# (0xFF56??) || call *0x??(%edi) (0xFF5F??) || call
# 								# *0x??(%ebx)
#                 # call *0x??(%edx) (0xFF52??) || call *0x??(%ecx)
# 								# (0xFF51??) || call *0x??(%edi) (0xFF57??) || call
# 								# *0x??(%eax) (0xFF50??)
...
# # FIXME: make sure that the interval fits the 32bits address for arm
# # and 16bits for thumb
# # status: works, fixme
# define nop
#     if ($argc > 2 || $argc == 0)
#         help nop
...
# # seems nasm shipping with Mac OS X has problems accepting input from
# # stdin or heredoc
# # input is read into a variable and sent to a temporary file which
# # nasm can read
# define assemble
...
#     if ($argc)
# 			    if ($64BITS == 1)
# 						    # argument specified, assemble instructions into
# 						# memory at address specified.
...
#     if ($argc)
#         # argument specified, assemble instructions into memory at
# 			# address specified.
#         shell ASMOPCODE="$(while read -ep '>' r && test "$r" != end ; do echo -E "$r"; done)" ; GDBASMFILENAME=$RANDOM; \
#         echo -e "BITS 32\n$ASMOPCODE" >/tmp/$GDBASMFILENAME ; /usr/bin/nasm -f bin -o /dev/stdout /tmp/$GDBASMFILENAME | /usr/bin/hexdump -ve '1/1 "set *((unsigned char *) $arg0 + %#2_ax) = %#02x\n"' >/tmp/gdbassemble ; /bin/rm -f /tmp/$GDBASMFILENAME
#         source /tmp/gdbassemble
...
# #     - Added the "skip" command. This will jump to the next
# #instruction after EIP/RIP without executing the current one.
# #       Thanks to @bSr43 for the tip to retrieve the current
# #instruction size.
# #
# #Version 7.4.3 (04/11/2011)
# #  - Modified "hexdump" command to support a variable number of lines
# #(optional parameter)
# #  - Removed restrictions on type of addresses in the "dd" command -
# #Thanks to Plouj for the warning :-)
# #   I don't know what was the original thinking behind those :-)
# #  - Modified the assemble command to support 64bits - You will need
# #to recompile nasm since the version shipped with OS X doesn't
# #supports 64bits (www.nasm.us).
# #   Assumes that the new binary is installed at /usr/local/bin -
# #modify the variable at the top if you need so. 
# #   It will assemble based on the target arch being debugged. If you
# #want to use gdb for a quick asm just use the 32bits or 64bits
# #commands to set your target.
# #      Thanks to snare for the warning and original patch :-)
# #  - Added "asm" command - it's a shortcut to the "assemble" command.
# #  - Added configuration variable for colorized prompt. Plouj reported
# #some issues with Ubuntu's gdb 7.2 if prompt is colorized.
# #
# #   Version 7.4.2 (11/08/2011)
# #    Small fix to a weird bug happening on FreeBSD 8.2. It doesn't
# #like a "if(" instruction, needs to be "if (". Weird!
...
# #    Added patch sent by sbz, more than 1 year ago, which I forgot to
# #add :-/
# #     This will allow to search for a given pattern between start and
# #end address.
# #     On sbz words: "It's usefull to find call, ret or everything like
# #that." :-)
# #    New command is "search"
# #
# #   Version 7.4 (20/06/2011) - fG!
# #    When registers change between instructions the colour will change
# #to red (like it happens in OllyDBG)
# #     This is the default behavior, if you don't like it, modify the
# #variable SHOWREGCHANGES
...
# #  Added the command rint3 and modified the int3 command. The new
# #command will restore the byte in previous int3 patch.
# #
# # Version 7.3.1 (29/06/2010) - fG!
# #  Added enablelib/disablelib command to quickly set the
# #stop-on-solib-events trick
# #  Implemented the stepoh command equivalent to the stepo but using
# #hardware breakpoints 
# #  More fixes to stepo
# #
# #Version 7.3 (16/04/2010) - fG!
# #  Support for 64bits targets. Default is 32bits, you should modify
# #the variable or use the 32bits or 64bits to choose the mode.
# #       I couldn't find another way to recognize the type of binary…
# #Testing the register doesn't work that well.
...
# #  Added the smallregisters function to create 16 and 8 bit versions
# #from the registers EAX, EBX, ECX, EDX
# #  Revised and fixed all the dumpjump stuff, following Intel
# #manuals. There were some errors (thx to rev who pointed the jle
# #problem).
# #  Small fix to stepo command (missed a few call types)
# #
# #   Version 7.1.7 - fG!
# #     Added the possibility to modify what's displayed with the
# #context window. You can change default options at the gdb options
# #part. For example, kernel debugging is much slower if the stack
# #display is enabled...
# #     New commands enableobjectivec, enablecpuregisters, enablestack,
# #enabledatawin and their disable equivalents (to support realtime
# #change of default options)
# #     Fixed problem with the assemble command. I was calling /bin/echo
# #which doesn't support the -e option ! DUH ! Should have used bash
# #internal version.
# #     Small fixes to colours...
# #     New commands enablesolib and disablesolib . Just shortcuts for
# #the stop-on-solib-events fantastic trick ! Hey... I'm lazy ;)
# #     Fixed this: Possible removal of "u" command, info udot is
# #missing in gdb 6.8-debian . Doesn't exist on OS X so bye bye !!!
# #     Displays affected flags in jump decisions
# #
# #   Version 7.1.6 - fG!
# #     Added modified assemble command from Tavis Ormandy (further
# #modified to work with Mac OS X) (shell commands used use full path
# #name, working for Leopard, modify for others if necessary)
# #     Renamed thread command to threads because thread is an internal
# #gdb command that allows to move between program threads
# #
# #   Version 7.1.5 (04/01/2009) - fG!
# #     Fixed crash on Leopard ! There was a If Else condition where the
# #else had no code and that made gdb crash on Leopard (CRAZY!!!!)
# #     Better code indention
# #
# #   Version 7.1.4 (02/01/2009) - fG!
# #     Bug in show objective c messages with Leopard ???
# #     Nop routine support for single address or range (contribution
# #from gln [ghalen at hack.se])
# #     Used the same code from nop to null routine
# #
# #   Version 7.1.3 (31/12/2008) - fG!
# #     Added a new command 'stepo'. This command will step a temporary
# #breakpoint on next instruction after the call, so you can skip over
# #     the call. Did this because normal commands not always skip over
# #(mainly with objc_msgSend)
# #
# #   Version 7.1.2 (31/12/2008) - fG!
# #     Support for the jump decision (will display if a conditional
# #jump will be taken or not)
# #
# #   Version 7.1.1 (29/12/2008) - fG!
# #     Moved gdb options to the beginning (makes more sense)
# #     Added support to dump message being sent to msgSend (easier to
# #understand what's going on)
# #
# #   Version 7.1
# #     Fixed serious (and old) bug in dd and datawin, causing
# #dereference of
...
# #     should only be controlled globally with context-on and
# #context-off
...
# #       2) removed the "clean up trace file" part, which is not needed
# #now,
...
# #     Fixed print_insn_type -- removed invalid semicolons!, wrong
# #value checking,
# #     Added TODO entry regarding the "u" command
# #     Changed name from gas_assemble to assemble_gas due to
# #consistency
# #     Output from assemble and assemble_gas is now similar, because i
# #made
# #     both of them to use objdump, with respect to output format
# #(AT&T|Intel).
# #     Whole code was checked and made more consistent,
# #readable/maintainable.
# #
# #   Version 6.2
# #     Add global variables to allow user to control stack, data and
# #code window sizes
# #     Increase readability for registers
# #     Some corrections (hexdump, ddump, context, cfp, assemble,
# #gas_asm, tips, prompt)
# #   
...
# #
# #   Version 6
# #     Added print_insn_type, get_insn_type, context-on, context-off
# #commands
