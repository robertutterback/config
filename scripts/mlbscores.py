...
#NAME
#mlbscores - report baseball scores in realtime by reading the ESPN
#MLB scoreboard webpage
#SYNOPSIS
#mlbscores [team]
#DESCRIPTION
#run the script with no arguments to see all the scores
#run the script with one argument to only show games with teams with
#that phrase in their name participating (case insensitive)
#team names are as they appear on the ESPN website (city names
#actually, and/or team names if ambiguous)
#the special phrase 'all' will show all games (useful when default
#team is hardcoded)
#use watch for realtime updates
#BUGS
#python's sgmllib.SGMLParser complains about ESPN's webpage as late as
#python 2.2.3
#I've seen ESPN put a 0 in the top of the 1st for a postponed game,
#making it appear as if it's in progress (this 0 is not rendered in a
#web browser)
#obviously, if ESPN changes their webpage enough, it will break this
#script
...
class ESPNMLBScoresParser(sgmllib.SGMLParser):
    #game data is stored like [['Away Team', '1', '2', '3', '4', '5',
    #'6', '7', '8', '9', 'R', 'H', 'E'], ['Home Team', '1', '2', '3',
    #'4', '5', '6', '7', '8', '9', 'R', 'H', 'E']]
    #1..9 are the runs scored that inning, and RHE are the respective
    #totals
    #everything is a string (all numeric, except 1..9 entries may be
    #'' (inning no yet completed) or 'X' (bottom of the 9th not
    #played))
    #the data is filled in by first creating all entries as '', then
    #filling them in one by one as they're reached when parsing the
    #html source
    #self.getGames() returns a list of these game data strutures
    #notes on ESPN html code
    #<div class=gameContainer> starts a game
    #<td class=teamLine>, first <a>'s data after that is away team
    #name
    #<td class=teamLine>, first <a>'s data after that is home team
    #name
    ...
    def __init__(self):
        ...
        #sgmllib.SGMLParser complains about ESPN's page as late as 2.2.3, but
        #doesn't as early as 2.3.3 (I don't know about anything between)
        verReq = [2,3,3]
    ...
    def _reset_vars(self):
        ...
        self.stat_index = 0  #0 ~ team name, 1..9 ~ runs in each inning, 10-12
        #~ RHE
    ...
    #self.team_index and self.stat_index now point to where the last
    #thing went; make them point to where the next thing will go
